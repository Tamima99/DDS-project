...
CREATE OR REPLACE PACKAGE Body predict_laptop_site1 AS
	...
	BEGIN
		For R in (select pid,absolute_value(memory, A) as df_memory, absolute_value(storage, B) as df_storage,
			...
			absolute_value(display, E) as df_display from specification1@site_link) LOOP
						 
			insert into difference_table1@site_link values(R.pid, R.df_memory, R.df_storage, R.df_cpu_speed, R.df_battery, 
			R.df_display);
		End Loop;		
	...
	loopCount int :=0;
	BEGIN 
		For R in (select * from difference_table1@site_link natural join 
		laptop natural join specification1@site_link order by df_memory) LOOP
	...
	BEGIN 
		For R in (select * from difference_table1@site_link natural join 
		laptop natural join specification1@site_link order by df_storage) LOOP
	...
	BEGIN 
		For R in (select * from difference_table1@site_link natural join 
		laptop natural join specification1@site_link order by df_battery) LOOP
	...
	BEGIN 
		For R in (select * from difference_table1@site_link natural join 
		laptop natural join specification1@site_link  order by price) LOOP
