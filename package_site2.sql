...
CREATE OR REPLACE PACKAGE Body predict_laptop_site2 AS
	...
	BEGIN
		For R in (select pid,absolute_value(memory, A) as df_memory, absolute_value(storage, B) as df_storage,
			...
			absolute_value(display, E) as df_display from specification2)LOOP
					
			insert into difference_table2 values(R.pid, R.df_memory, R.df_storage, R.df_cpu_speed, R.df_battery, 
			R.df_display);
		End Loop;		
	...
	loopCount int :=0;
	BEGIN                                  
		For R in (select * from difference_table2 natural join 
										
		laptop natural join specification2 order by df_memory) LOOP
	...
	BEGIN									 
		...
		laptop natural join specification2 order by df_storage) LOOP
	...
	BEGIN								    
		...
		laptop natural join specification2 order by df_battery) LOOP
	...
	BEGIN								
		...
		laptop natural join specification2 order by price) LOOP
